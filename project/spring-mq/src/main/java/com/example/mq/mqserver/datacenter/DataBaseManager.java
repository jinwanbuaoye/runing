...
import com.example.mq.SpringMqApplication;
...
public class DataBaseManager {
    ...
    // 针对数据库进行初始化
    /*
    例如，现在把 broker server 部署到一个新的服务器上，显然，此时是没有数据库.
    就需要让broker server启动的时候,自动的把对应的数据库创建好~~
    但是如果是一个已经部署过的机器，broker server重启了，就会发现，数据库已经有了，此时不做任何数据库相关操作.
     */
    public void init() {
        // 手动的获取到 MetaMapper
        metaMapper = SpringMqApplication.context.getBean(MetaMapper.class);
